...
// Binance P2P - Use Vercel Proxy as primary (Cloudflare IPs are blocked by Binance WAF)
async function fetchBinanceRate(): Promise<number | null> {
    const VERCEL_PROXY = 'https://giausdtvn-waf.vercel.app/api/binance';
    ...
    try {
        ...
        console.error('Binance Proxy failed, trying direct fallback as last resort:', e);
    }

    // DIRECT FALLBACK (Usually blocked by Cloudflare WAF, but kept as last resort)
    try {
        const response = await fetchWithRetry('https://p2p.binance.com/bapi/c2c/v2/friendly/c2c/adv/search', {
            ...
            body: JSON.stringify({
                "fiat": "VND", "page": 1, "rows": 10, "tradeType": "BUY", "asset": "USDT",
                "publisherType": "merchant", "classifies": ["mass", "profession", "fiat_trade"]
            }),
        });
        ...
        if (response.ok) {
            ...
            if (Array.isArray(data?.data) && data.data.length > 0) {
                ...
                if (bestAd?.adv?.price) {
                    const price = parseFloat(bestAd.adv.price);
...
// AlchemyPay API - Use Vercel Proxy as primary
async function fetchAlchemyRate(): Promise<number | null> {
    const VERCEL_PROXY = 'https://giausdtvn-waf.vercel.app/api/alchemy';
    ...
    try {
        ...
        console.error('Alchemy Proxy failed, trying direct fallback as last resort:', e);
    }
    ...
    try {
        ...
        const response = await fetchWithRetry(url, {
            ...
            body: JSON.stringify({ "crypto": "USDT", "fiat": "VND", "side": "buy", "amount": "8100601", "alpha2": "VN", "network": "BSC" }),
        });

        if (response.ok) {
            const data = await response.json();
            let rate = 0;
            if (data?.data?.cryptoPrice) rate = parseFloat(data.data.cryptoPrice);
            else if (data?.data?.fiatAmount && data?.data?.cryptoAmount) rate = parseFloat(data.data.fiatAmount) / parseFloat(data.data.cryptoAmount);

            if (rate > 1000) return rate;
        }
    } catch (e) {
        console.error('Alchemy direct fallback failed:', e);
    }
    return null;
}
